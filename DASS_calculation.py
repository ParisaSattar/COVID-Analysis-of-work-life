...
city_index = []
for city_value in range (len(survey_data)):
    if (survey_data.iloc[city_value]['City'] != "Karachi"):
        city_index.append(city_value)
        
for i in range (len(city_index)):
    value = city_index[i]-i
    survey_data.drop([survey_data.index[value]],inplace = True)
...
'''
df1 = pd.DataFrame(depression_variable)
df2 = pd.DataFrame(anxiety_variable)
df3 = pd.DataFrame(stress_variable)

df = pd.concat([df1,df2,df3],axis = 1)

df.to_csv("DASS-21.csv")
'''
